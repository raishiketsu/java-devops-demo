...
ChatGPTがはじめとする生成AIがすごく話題になっている中で、画像生成AIのStable dffusionをcGPU環境に実行するのを実施して見ようと思います。

### Stable dffusionについて
ChatGPTを開発したOpenAI社にも、DALL-E2画像生成AIを出しています。それと比べてStable dffusionはオープンソースなサービスで、GPUがあれば誰でも無料で使えます。
利用料金がかからない反面、オープンソースのため自分でセットアップする必要があります。
Nvidia のGPUが付いているPCにセットアップするのが一般的な利用シーンですが、もしNvidia のGPUが付いてPCが持っていない場合、高額なPCを新規で買うより、従量課金で使った分だけの料金でクラウド上で実行するのも一つの案です。
さらに、AlibabaCloudのcGPUを使えば、複数の機械学習タスクを効率よく実行したり、複数の人の環境を同時実行したりすることができます。
